...
R_eh1 = P.theta1 * (1/(P.gamma_h +P.mu_h)) * P.beta_h * (conditions(1)/N) * (((P.theta1 * calc_b_T(P, conditions))/(N * P.theta1))*P.theta1);
R_eh2 = P.theta2 * (1/(P.gamma_h +P.mu_h)) * P.beta_h * (conditions(1)/N) * (((P.theta2 * calc_b_T(P, conditions))/(N * P.theta2))*P.theta2);
R_h = R_eh1+R_eh2;
% (conditions(1)/N) = (S_h * theta1)/(Nh * theta1)
Nv = conditions(5)+conditions(6)+ conditions(7);
R_v = (1/(P.nu_v + P.mu_v))*(P.nu_v/P.mu_v) * P.beta_v * conditions(5)/Nv *(calc_b_T(P, conditions)/(Nv));
